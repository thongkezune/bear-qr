# 🐻 Cẩm nang sử dụng: Cỗ máy Sản Xuất Mã Gấu (Bulk QR Generator)
...
- **Tuỳ biến Đích đến:** Nối mã QR vào đường dẫn thử nghiệm (`localhost`) hoặc Website sống (`bearqr.com`).
...
```bash
node scripts/generate_qrs.mjs --count 50 --url https://bearqr.vn/m/
```

_(Lưu ý: Bạn sửa chữ `bearqr.vn` thành tên miền bạn đã mua)_
...
2. Tìm đến dòng số **20**:
   ```javascript
   let logoUrl = "https://bearqr.com/logo.png";
   ```
